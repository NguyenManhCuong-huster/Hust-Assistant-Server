...
export const getMeta = async (req, res, next) => {
  try {
    ...
    if (!row) return res.status(404).json({ success: false, message: 'Không tìm thấy hoặc không có quyền.' });
    res.json({
      success: true,
      data: {
        id:            row.id,
...
export const download = async (req, res, next) => {
  try {
    ...
    if (!row) return res.status(404).json({ success: false, message: 'Không tìm thấy hoặc không có quyền.' });
    if (!row.storage_path) {
      return res.status(404).json({ success: false, message: 'File chưa được tải xuống. Vui lòng thử lại sau.' });
    }
    ...
    res.setHeader(
      'Content-Disposition',
      `${isPreviewable ? 'inline' : 'attachment'}; filename*=UTF-8''${fileNameForHeader(row.file_name)}`,
    );
