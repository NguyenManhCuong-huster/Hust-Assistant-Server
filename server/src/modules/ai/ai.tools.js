...
import { extractAttachmentTextByFileNameInIds } from '../attachments/attachments.text.js';
...
import {
  ...
  isSearchToolName,
  dispatchSearchTool,
} from './ai.search-tools.js';
import { buildReferenceSystemNote } from './ai.references.js';
...
export const buildToolSystemNote = () => {
  const lines = [
    ...
    '',
    'MINH BẠCH KHI DÙNG TOOL — BẮT BUỘC cho MỌI tool:',
    '  Trong câu trả lời CUỐI CÙNG (sau khi (các) tool đã chạy xong), MỞ ĐẦU bằng',
    '  1–2 câu cho user biết RÕ 3 điều, rồi mới tới nội dung chi tiết:',
    '    (a) đã dùng CÔNG CỤ gì — gọi tên thân thiện kèm tên tool trong ngoặc,',
    '        vd "tìm email (search_emails)", "tra web (web_search)", "đọc file (read_attachment)";',
    '    (b) với THAM SỐ HIỆU LỰC nào — nếu kết quả tool có `effective_params` thì ĐỌC',
    '        và báo ĐỦ, GỒM CẢ mặc định server tự áp khi bạn KHÔNG truyền (đừng chỉ nói',
    '        các tham số bạn điền tay). Diễn giải giá trị null/bỏ trống cho dễ hiểu:',
    '          • query = null  → "không lọc từ khóa, lấy mới nhất"',
    '          • from_date/to_date = null → "không giới hạn ngày"',
    '          • kind = null   → "cả tin tức lẫn kế hoạch"',
    '          • limit / num   → "tối đa N kết quả"',
    '    (c) TÓM TẮT kết quả thu được — vd "thấy 3 email", "được 2 kết quả", "ghi 5 môn".',
    '  Ví dụ: "Mình đã tìm email với từ khóa \'thầy Hùng\' (search_emails) và thấy 3 email tuần này: ..."',
    '  Ví dụ (không truyền tham số): "Mình lấy email mới nhất (search_emails: không lọc từ khóa,',
    '         không giới hạn ngày, tối đa 10 email), thấy 5 email: ..."',
    '  Ví dụ: "Mình tra web từ khóa \'học bổng Erasmus 2026\' (web_search), được 2 kết quả chính: ..."',
    '  Ví dụ: "Mình đã đọc file lich_thi.pdf (read_attachment); nội dung chính: ..."',
    '  Ví dụ: "Mình đã ghi điểm 5 học phần kỳ 20251 (create_grades): 3 môn mới, 2 cập nhật."',
    '  Nếu gọi NHIỀU tool, liệt kê ngắn từng tool đã dùng + tham số + kết quả tương ứng.',
    '  Mục tiêu: đọc xong câu trả lời, user LUÔN biết AI đã DÙNG GÌ, với THAM SỐ GÌ, và',
    '  THU ĐƯỢC GÌ — tuyệt đối không giấu hay bỏ qua bước này.',
    '',
