...
import { normalizeCredits, normalizeLetter } from '../utils/validators.js';
...
export const createGrade = async (userId, fields) => {
  const { semester, course_code, course_name, course_name_en = null } = fields;
  const credits = normalizeCredits(fields.credits);
  const letter  = normalizeLetter(fields.letter_grade);
...
export const replaceGrade = async (id, fields) => {
  const { semester, course_code, course_name, course_name_en = null } = fields;
  const credits = normalizeCredits(fields.credits);
  const letter  = normalizeLetter(fields.letter_grade);
