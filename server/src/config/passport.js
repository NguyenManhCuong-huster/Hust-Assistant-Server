...
// ── Strategy 1: google-login — login only, no email access ───
...
passport.use('google-account', new GoogleStrategy(
  {
    ...
    // offline access → refresh_token included in callback
    accessType:        'offline',
  ...
  async (req, accessToken, refreshToken, profile, done) => {
    ...
    try {
      ...
      // Encrypt both tokens as a JSON blob
      const tokenBlob = encrypt(JSON.stringify({
        access_token:  accessToken,
        refresh_token: refreshToken ?? null, // null if user already authorized before
      }));
...
// ── Strategy 3: microsoft-account — link Outlook ─────────────
