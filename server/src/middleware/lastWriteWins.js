...
// Dùng: router.put('/:id', requireAuth, lwwCheck('tasks'), handler)
export const lwwCheck = (table) => async (req, res, next) => {
  const clientModTime = req.headers['x-client-mod-time'] ?? req.body?.mod_time;
  if (!clientModTime) return next(); // Không gửi mod_time → bỏ qua kiểm tra
  ...
  try {
    ...
    if (!result.rows[0]) return next(); // 404 xử lý ở route
    ...
    req.serverRecord = serverRecord; // Cache lại để route không query lại
    next();
  } catch (err) {
    next(err);
  }
